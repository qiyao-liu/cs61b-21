...
public class Model extends Observable {
    ...
    public boolean tilt(Side side) {
        ...
        // Set the viewing perspective of the board to the specified side
        board.setViewingPerspective(side);

        // Iterate through each column to perform the tilt operation
        for (int col = 0; col < size(); col++) {
            // Move all tiles up without merging
        ...
        // Check if the game is over and notify observers if the board has changed
        if (changed) {
            checkGameOver();
            setChanged();
        }
    ...
    private boolean moveAllTilesUpWithoutMerge(int col) {
        boolean changed = false;
    ...
    private boolean mergeIfNecessary(int col) {
        boolean changed = false;
        ...
        for (int col = 0; col < b.size(); col++) {
            for (int row = 0; row < b.size(); row++) {
                if (emptySpaceExists(b)) {
                    ...
                    if (col + 1 < b.size() && b.tile(col, row).value() != 0 && b.tile(col, row).value() == b.tile(col + 1, row).value()) {
                        return true;
                    }
                    if (row + 1 < b.size() && b.tile(col, row).value() != 0 && b.tile(col, row).value() == b.tile(col, row + 1).value()) {
                        return true;
                    }
                }
            }
